<template>
    <div class="card-sub-product">
        <img :src="imageUrl">
        <div id="circle">
            <div>
                <h2>tot</h2>
                <h2 id="money">€ {{ amount }}</h2>
                <h2>subsidie</h2>
            </div>
        </div>
        <h1>{{ title }}</h1>
        <p>{{ brand }}</p>
        <p>4 tot 6KW jank op die handel</p>
        <img id="label" src="../assets/icon/label.png">
        <div id="statements">
            <div>
        ...
        <div id="button-invoice">
            <button @click="scrollTo('contact-form-container')">Offerte aavragen</button>
        </div>
    </div>
</template>
